...
if(-Not ($args[0] -eq "-AfterReboot")) {
    ...
    $Arguments = '-Command "& { . '+$PSCommandPath+' -AfterReboot }"'
    $Action = New-ScheduledTaskAction -Execute powershell.exe -Argument $Arguments
    $Option = New-ScheduledTaskSettingsSet -AllowStartIfOnBatteries -WakeToRun
    $Trigger = New-JobTrigger -AtLogOn
    Register-ScheduledTask -TaskName Resume -Action $Action -Trigger $Trigger -Settings $Option -RunLevel Highest

    Restart-Computer

}
else {
    Write-Host("Creating directory for the distribution");
    ...
    Write-Host("Extracting .tar package");
    docker export --output=$DistroDir+"\"+$DockerExportTarName $DockerContainerLocal

    Write-Host("Registering created distro in WSL");
